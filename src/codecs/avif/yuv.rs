    ...
    i32: AsPrimitive<V>,
{
    ...
    for (((y_src, &u_src), &v_src), rgb_dst) in y_iter.zip(u_plane).zip(v_plane).zip(rgb_chunks) {
        ...
        let r = qrshr::<PRECISION, BIT_DEPTH>((t1 + cr_value) * r_coef);
        let b = qrshr::<PRECISION, BIT_DEPTH>((t1 - cr_value) * r_coef);
        let g = qrshr::<PRECISION, BIT_DEPTH>((y_value1 + cb_value) * r_coef);
